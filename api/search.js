...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Perform the search
    const results = await performSearch(searchParams, limit);
    ...
    const formattedResults = results.map(item => ({
      ...
      enhanced_summary: item.enhanced_summary,
      key_insights: item.key_insights,
      topics: item.topics,
      people_mentioned: item.people_mentioned,
      video_url: item.video_url,
      extracted_text: item.extracted_text ? item.extracted_text.substring(0, 200) + '...' : null,
      processed_date: item.processed_date,
      confidence_score: item.confidence_score,
      relevance_score: item.rank || 0
    }));
