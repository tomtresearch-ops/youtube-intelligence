...
export async function GET() {
  try {
    console.log('Stats API called - initializing database...');
    const database = await initDatabase();
    
    console.log('Database initialized, fetching stats...');
    ...
    const [
      ...
      // Total Knowledge
      database.get('SELECT COUNT(*) as count FROM processed_content'),
      
      // This Week
      database.get('SELECT COUNT(*) as count FROM processed_content WHERE processed_date > datetime("now", "-7 days")'),
      
      // Average Confidence
      database.get('SELECT AVG(confidence_score) as avg FROM processed_content WHERE confidence_score IS NOT NULL'),
      
      // Total Cost
      database.get('SELECT SUM(processing_cost) as total FROM processed_content'),
      
      // YouTube Videos
      database.get('SELECT COUNT(*) as count FROM processed_content WHERE content_type = "youtube"'),
      
      // Other Content
      database.get('SELECT COUNT(*) as count FROM processed_content WHERE content_type = "ocr"'),
      ...
      database.get('SELECT (COUNT(CASE WHEN confidence_score >= 0.8 THEN 1 END) * 100.0 / COUNT(*)) as rate FROM processed_content WHERE confidence_score IS NOT NULL')
    ]);
